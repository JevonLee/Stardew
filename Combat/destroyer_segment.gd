...
class_name DestroyerSegment
## 机械蠕虫身体段：链式跟随前一段，接触伤害；攻击身体伤害转嫁到头部

var leader: Node2D
var spacing: float = 44.0
var head_ref: Node = null ## 头部（伤害转嫁目标）
var contact_damage: int = 15
...
func _physics_process(delta: float) -> void:
	...
	if to_leader.length() > spacing:
		velocity = to_leader.normalized() * 380.0
	else:
