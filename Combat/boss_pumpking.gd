extends Boss
## 南瓜王：霜月Boss——漂浮追击+冲撞，发射火焰南瓜弹，半血狂暴

const BOLT = preload("res://Combat/enemy_bolt.tscn")

@export var gift_drop: Item ## 特殊掉落（南瓜）
...
func _physics_process(delta: float) -> void:
	...
	if not chasing:
		...
		match phase:
			Phase.FLOAT:
				# 漂浮逼近
				velocity = to_player.normalized() * 85.0 * boost + Vector2(sin(anim_time * 2.0), cos(anim_time * 2.0)) * 45.0
				# 周期发射火焰南瓜弹
				shoot_timer -= delta
				if shoot_timer <= 0.0:
...
func _shoot() -> void:
	...
	if sprite_node:
		sprite_node.modulate = Color(1.0, 0.6, 0.2, 1.0) # 火焰橙弹幕
...
func _on_death() -> void:
	...
	Global.show_message("南瓜王被击败了！万圣节的狂欢结束了！")
