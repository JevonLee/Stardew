extends Boss
## 圣诞坦克：霜月Boss——重型慢速逼近+3发扇形火箭弹+冲撞，半血狂暴
...
func _physics_process(delta: float) -> void:
	...
	anim_time += delta
	if sprite.vframes > 0:
		sprite.frame = int(anim_time * 5.0) % sprite.vframes
	var dist := 1e9
	if player:
	...
	if not chasing:
		velocity = Vector2(sin(anim_time * 2.0), cos(anim_time * 2.0)) * 25.0
	else:
		...
		match phase:
			Phase.FLOAT:
				# 重型慢速逼近
				velocity = to_player.normalized() * 70.0 * boost + Vector2(sin(anim_time * 1.5), cos(anim_time * 1.5)) * 30.0
				# 3发扇形火箭弹
				shoot_timer -= delta
				if shoot_timer <= 0.0:
					shoot_timer = 3.2 if not boost > 1.0 else 2.0
					_shoot_volley()
				phase_timer -= delta
				if phase_timer <= 0.0:
					phase = Phase.DASH
			...
			Phase.DASH:
				...
				if phase_timer <= 0.0:
					phase = Phase.FLOAT
					phase_timer = dash_cooldown * (0.6 if boost > 1.0 else 1.0)
...
func _on_death() -> void:
	...
	Global.show_message("圣诞坦克被击败了！霜月的装甲军团覆灭了！")
